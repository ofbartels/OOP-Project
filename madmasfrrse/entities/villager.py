...
class Villager:
    def __init__(self, x, y, sprite_path):
        ...
        self.sprite = pygame.transform.scale(original_sprite, (int(original_sprite.get_width() * 0.6), int(original_sprite.get_height() * 0.6)))
        self.speed = random.randint(1, 3)
        self.target_tile = None
        self.interaction_time = time.time()
        self.hangout_duration = 5
        self.visible = True
    ...
    def move_towards_target(self):
        if self.target_tile:
            direction = (self.target_tile.world_x - self.world_x, self.target_tile.world_y - self.world_y)
    ...
    def react_to_enemies(self, enemies):
        for enemy in enemies:
            if ((self.world_x - enemy.world_x)**2 + (self.world_y - enemy.world_y)**2)**0.5 < 50:
                self.run_away_from()
