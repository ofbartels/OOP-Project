...
class WizardTower(DefenseTower):
    upgrade_costs = [300, 650, 1000]
    wizard_sprite = 'assets/sprites/defense_towers/wizard/wizard_bullet.png'

    def __init__(self, grid_x, grid_y, building_type='wizard', level=1):
    ...
    def fire(self, enemies):
        ...
        if nearest_enemy and self.distance_to(nearest_enemy) < self.radius:
            self.projectiles.append(Projectile((self.world_x, self.world_y), nearest_enemy, 10, 'assets/sprites/defense_towers/wizard/wizard_bullet.png'))
