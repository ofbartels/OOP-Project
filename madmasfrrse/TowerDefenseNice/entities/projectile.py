...
class Projectile:
    def __init__(self, start_pos, target, damage, sprite_path, speed=5, gravity=0.2, vertical_velocity=-5):
        ...
        self.image = self.load_and_scale_image(sprite_path, 0.75)
        self.rotated_image = None
        self.rect = self.image.get_rect()

    def load_and_scale_image(self, sprite_path, scale_factor):
        image = pygame.image.load(sprite_path).convert_alpha()
        new_size = (int(image.get_rect().width * scale_factor), int(image.get_rect().height * scale_factor))
